...
kable(papeR::summarize(bikeshare, type = "factor", variables = "month",
                       test = FALSE)) %>%
  kable_styling(full_width = F)
kable(papeR::summarize(bikeshare, type = "factor", variables = "date",
                       test = FALSE)) %>%
  kable_styling(full_width = F)
...
- Overall, the yearly distribution of temperature looks similar across years 2011-2012 in Washington D.C.
...
ggplot(bikeshare, aes(x = temp, y = cnt )) +
    ...
    scale_fill_gradient2(
      name = "Count of bikes rented out",
      low = 'purple',  
      high = 'purple4',  
      na.value = "grey80") +
...
- We can view the data.frame `ny_school_enrollment_socio`:
<br>

```{r, result = 'asis', echo = F, message = F, warning = F}
url2 <- 'https://bcdanl.github.io/data/NY_school_enrollment_socioecon.csv'
ny_school_enrollment_socio <- read_csv(url2)
rmarkdown::paged_table(ny_school_enrollment_socio)
```
...
- Here are some details about the data.frame, `ny_school_enrollment_socio`:

- The geographic and time units of observation (row) in the data.frame, `ny_school_enrollment_socio`, are *New York county* and *year*.
...
- The following describes the variables:
  ...
  -  A county's college enrollment level can be represented by an *overall* tendency of that county's residents to be enrolled in college.

  - Which county does have a higher level of college enrollment? 
<br><br>
...
- To represent a level of college enrollment of population in a county, I choose variable, `c04_010`, a **percent** of population enrolled in **public** college or graduate school.

- To represent a level of college enrollment of population in a county, I choose variable, `c06_010`, a **percent** of population enrolled in **private** college or graduate school.
...
```{r, echo = T, eval = F, message = F, warning = F}
ggplot(ny_school_enrollment_socio,
...
```{r, echo = F, eval = T, warning= F, message= F}
ggplot(ny_school_enrollment_socio, aes(x = 100-d01_024, 
                                       y = c02_010 )) +
    geom_hex()  +
    geom_smooth(aes(color = "Linear"),
                method = lm,
...
- For the same level of the percentage of population 45 to 64 years without Bachelor's degree, a level of educational attainment of **male** population 45 to 64 years may be associated with a **higher** level of college enrollment than that of **female** population 45 to 64 years.
...
- For the same level of the percentage of population 45 to 64 years without Bachelor's degree, a level of educational attainment of **male** population 45 to 64 years may be associated with a higher level of **private** college enrollment than that of **female** population 45 to 64 years.
