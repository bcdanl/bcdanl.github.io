...
cv_entries %>% 
  ...
  mutate(what = str_c("<span class='cv-entry-title' style = 'font-weight:bold'>", what, "</span><br>", 
                      where)) %>% 
  select(year, what) %>% 
...
cv_entries %>% 
  ...
  mutate(additional_info = str_c("<span class='cv-entry-title' style = 'font-weight:bold'>", additional_info, "</span><br class='work-break'>")) %>% 
...
cv_entries %>% 
  ...
  mutate(what = str_c("<span class='cv-entry-title' style = 'font-weight:bold'>", what, "</span><br class='work-break'>", where, "<br>", additional_info)) %>% 
  arrange(desc(date)) %>% 
  select(year, what) %>% 
  kbl(padding = 0, col.names = NULL, escape = FALSE) %>%
  kable_styling( bootstrap_options = c("hover", "condensed"))   %>% 
