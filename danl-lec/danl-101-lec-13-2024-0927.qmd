...
-  `tidyverse` verbs work well with the **pipe**, `|>`, because the first argument of a `tidyverse` verb is a `data.frame` and the output is a `data.frame`, 
...
- The **pipe** (`|>`) takes the thing on its left and passes it along to the function on its right so that
  ...
  - e.g., `left_join(DATA_FRAME_1, DATA_FRAME_2)` is equivalent to `DATA_FRAME |>  left_join(DATA_FRAME_2)`.
...
<div style="text-align: center; width: 53%; margin: auto;">
