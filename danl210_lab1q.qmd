...
- Write a function with `def` and `.sort_values()`. 
...
## Q2a.
- Report the mean, median, minimum, maximum, and standard deviation of the each of the following variables:
  - `lifeExp`
  - `pop` 
  - `gdpPercap`
...
- Write a function with `def` and `if` 
- Then, use the `def`ined function in `apply()` to find the top 5 rich counties in NY for each year. 
