...
Provide both (1) ggplot code and (2) a simple comment to describe how the yearly trend of total `Contribution` varies by `Energy` and `Status`.

```{r}
climate_finance %>% 
  group_by(Energy, Status, Year) %>% 
  summarise(tot = sum(Contribution, na.rm = T)) %>% 
