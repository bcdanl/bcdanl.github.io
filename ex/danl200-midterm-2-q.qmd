...
q2e <- h1b %>%
  ...
  group_by(SOC_TITLE) %>%
  mutate(tot = n()) %>% 
  mutate(n_certi = ifelse(CASE_STATUS == "CERTIFIED", 1, 0),
         n_certis = sum(n_certi),
         prop_certi = 100*n_certis/tot) %>% 
  mutate(n_certiw = ifelse(CASE_STATUS == "CERTIFIED-WITHDRAWN", 1, 0),
         n_certiws = sum(n_certiw),
         prop_certiw = 100*n_certiws/tot) %>% 
  mutate(n_denied = ifelse(CASE_STATUS == "DENIED", 1, 0),
         n_denieds = sum(n_denied),
         prop_denied = 100*n_denieds/tot) %>% 
  mutate(n_w = ifelse(CASE_STATUS == "WITHDRAWN", 1, 0),
         n_ws = sum(n_w),
         prop_w = 100*n_ws/tot) %>% 
  select(SOC_TITLE, prop_certi, prop_certiw, prop_denied, prop_w, tot) %>% 
  distinct() %>% 
  arrange(-prop_certi)

```

- In the above, I provided just one R code for the answer.
