...
The optimal allocation of resources obtained from the Social Planner's problem plays an important role in making public policies. It is a benchmark that the government wants to achieve by implementing public policies.
...
$$
Therefore, $h^{o} = 1$ and $\{(x_{1}^{o}, x_{2}^{o}): Y_{1} + Y_{2} = x_{1}^{*} + x_{2}^{*}\}$.
...
- Dividing $SW(h)$ into benefit ( $B(h) = h(4-h)$ ) and cost ( $C(h) = h^{2}$ ) is helpful: 
...
$$
\begin{align}
\frac{d \mathcal{L}}{d h} &= h - 4h = 0 \tag{FOC-G-h}\\
...
Claim that $\hat{c} = 5 - a$, where $AP(\hat{c}) = a = MC$ is the total number of cows if the individual herder chooses how many of his or her cows to be grazed on the common.
$$
\frac{f(\hat{c})}{\hat{c}} = 5 - \hat{c} = a.
$$

Suppose the individual herder bought a cow, so that there is $c = \hat{c} + 1$ number of cows in the common. The average product of a cow is then:

$$
AP(\hat{c} + 1) = 4 - \hat{c} < 5 - \hat{c} = a = MC.
$$

Therefore, the individual herder does not have incentive to buy a cow when there is $c = \hat{c}$ number of cows in the common.
