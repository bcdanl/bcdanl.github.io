...
The town of Geneseo has two residents: Boden and Tromp. The town currently funds its sustainability department solely from the individual contributions of Boden and Tromp. 

Boden and Tromp have their own utility functions, $U_{B}$ and $U_{T}$ respectively, over private goods ( $X$ ) and total sustainability workers ( $M$ ) of the forms:
...
Boden and Tromp both have income of $\$100$, and the price of both the private good and the sustainability worker is $\$1$. Therefore, they are limited to providing between 0 and 100 sustainability workers.

<br>

## Q1a.
- How many sustainability workers are hired if the Geneseo government does not intervene? 
  - How many are paid for by Boden? By Tromp?
  - Describe the Boden's and the Tromp's decisions on provision of sustainability workers using a graph in the $(M_{B}, M_{T})$ space.

<br>

Taking the Tromp's action and the prices of the private good and the sustainability worker as given, Boden maximizes his utility:
...
Taking the Boden's action and the prices of the private good and the sustainability worker as given, Tromp maximizes his utility:
...
Both Boden's and Tromp's optimal decisions on how many sustainability workers to hire are characterized by (Action-B) and (Action-T), respectively. Solving the two unknowns---$M_{B}$ and $M_{T}$---in the two equations---(Action-B) and (Action-T)---gives us:
...
The following describes Boden's and Tromp's reactions to the other individual's choice on the number of sustainability workers to hire:

```{r}

df <- data.frame(m_i = seq(0, 150, 5),
                 boden = 50 - seq(0, 150, 5),
                 tromp = 50 - 1.5*seq(0, 150, 5)
)

df <- df %>%
  rename(`Boden's Reaction to Tromp` = boden,
         `Tromp's Reaction to Boden` = tromp) %>% 
  pivot_longer(`Boden's Reaction to Tromp`:`Tromp's Reaction to Boden`,
               names_to = "Reaction",
               values_to = "fn")
...
ggplot(df, aes(x = m_i, y = fn )) +
  ...
  labs(x = "Number of Tromp's Sustainability Workers",
       y = "Number of Boden's Sustainability Workers",
  ) +
...
Taking both Boden's and Tromp's budget constraints as given, the Social Planner chooses $M_{B}$ and $M_{T}$ to maximize the social welfare $SW = U_{B} + U_{T}$:
$$
\begin{align}
...
The Social Planner's allocation of the number of Boden's and Tromp's sustainability workers is characterized by (Action-SW-B) and (Action-SW-T). Solving the two unknowns---$M_{B}$ and $M_{T}$---in the two equations---(Action-SW-B) and (Action-SW-T)---gives us:
...
The following plots (Action-SW-B) and (Action-SW-T), describing how the Social Planner allocates Boden's and Tromp's sustainability workers:

```{r}

df <- data.frame(m_i = seq(0, 180, 5),
                 boden = (1/8)*( 500 - 3*seq(0, 180, 5) ),
                 tromp = (1/4)*( 500 - 9*seq(0, 180, 5) )
)
...
- Notice that Boden's contribution to sustainability goes down as Tromp's contribution goes up, and vice versa. This implies that both individuals have incentive to free ride on each other's.
