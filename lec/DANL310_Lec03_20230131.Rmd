...
- We can use `Sys.Date()` to the date field in YAML to set the date today.
...
- We can also use `Sys.time()` to the date field in YAML to set the date today.
...
---
# Get Started with `ggplot` visualization
### <p style="color:#00449E"> ggplot workflow </p>

1. Tidy data we need.


2. Tell the `ggplot()` function what our data is. 
  - `ggplot(data = ...)`


3. Tell `ggplot()` what relationships we want to see.
  - The `ggplot(mapping = aes(...))` 
  - `p <- ggplot(data = ... , aes(...))`
  
  
4. Tell `ggplot()` how we want to see the relationships in our data.
  - Choose a `geom_*()`.
  
  
5. Layer on `geoms` as needed, by adding them to the `p` object one at a time.


6. Use some additional functions to adjust scales, labels, tick marks, titles. 
  - `scale_*()`, `labs()`, `guides()`, `theme()`
...
- We can also set aesthetic mapping manually outside `aes()` function and inside `geom_*()` function
```{r}
p <- ggplot(data = gapminder,
            mapping = aes(x = gdpPercap,
                          y = lifeExp))
