...
Step 1. Download the template files from Byeong-Hak's GitHub repository.
...
```{r, eval = T, echo = F, out.width='77.5%', fig.align='center'}
knitr::include_graphics("../lec_figs/rstudio-terminal0.gif")
```
...
Step 2. Arrange the files. 
  ...
  1. Keep the `danl-web-template` subfolder and (2) the R project file, `YOUR_GITHUB_USERNAME.github.io.Rproj`.
    - Delete all the other files in the  `YOUR_GITHUB_USERNAME.github.io` folder, or move them to somewhere outside of the `YOUR_GITHUB_USERNAME.github.io` folder.

  2. Move all the files in the `danl-web-template` folder to the  `YOUR_GITHUB_USERNAME.github.io` folder.
  
  3. Delete the empty subfolder, `danl-web-template`.
...
Step 3. Render the site.
  1. From RStudio, open the files `index.qmd` and `_quarto.yml`.
  2. Replace `YOUR_NAME` with your preferred name.
  3. Replace `https://github.com/` with `https://github.com/YOUR_GITHUB_USERNAME`.
  4. Save the files, `index.qmd` and `_quarto.yml`.


Step 4. In Terminal within RStudio,
  - Run `quarto render`
...
Step 5. Add, commit, and push the updated files to your GitHub repository.
...
Step 6. Go to your website `r emo::ji('smile')`
...
- *1*. Modify `index.qmd` as you wish.
