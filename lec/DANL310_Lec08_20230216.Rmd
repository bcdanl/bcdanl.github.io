...
- We have used `scale_x_log10()`, `scale_x_continuous()` and other` scale_` functions to adjust axis labels. 
...
- Why are there so many `scale_` functions? 
...
- Every aesthetic mapping has a scale. 
  ...
  - If we want to adjust how that scale is marked or graduated, then we use a `scale_` function.
...
- Many scales come with a *legend* or key to help the reader interpret the graph. These are called `guides`. 
  ...
  - Perhaps the most common use case is to make the legend disappear, as it is sometimes superfluous. 
