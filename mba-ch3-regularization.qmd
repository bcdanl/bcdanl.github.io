...
$$\sum_i^n \big(Y_i - \beta_0 -  \sum_j^p \beta_j x_{ij}\big)^2 + \lambda \sum_j^p \beta_j^2 = \text{SSR} + \lambda \sum_j^p \beta_j^2$$
...
$$\sum_i^n \big(Y_i - \beta_0 -  \sum_j^p \beta_j X_{ij}\big)^2 + \lambda \sum_j^p\vert  \beta_j \vert = \text{SSR} + \lambda \sum_j^p \vert \beta_j \vert$$
...
- Lasso regression __performs variable selection__ yielding __sparse__ models, which only includes a small subset of the available variables that are relevant to predicting the outcome of interest.
...
$$\sum_i^n \big(Y_i - \beta_0 -  \sum_j^p \beta_j X_{ij}\big)^2 + \lambda \sum_j^p\vert  \beta_j \vert = \text{SSR} + \lambda \sum_j^p \log(\, 1 + \vert \beta_j \vert\,)$$
...
- For either ridge, lasso, gamma lasso or elastic net: __we should consider standardizing our data__
...
- How should we choose `k`?
  ...
  - Note that the variance of the `k`-CV estimate of average OOS deviance is the variance on each left-out fold 
